...
def build_optim(model, opt, checkpoint):
    ...
    if opt.train_from:
        ...
        # the method optim.set_parameters(models.parameters()) will overwrite
        # optim.optimizer, and with ith the values stored in
        # optim.optimizer.state_dict()
        saved_optimizer_state_dict = optim.optimizer.state_dict()
    else:
        optim = Optimizer(
            opt.optimizer, opt.learning_rate, opt.max_grad_norm,
    ...
    # models.paramters() as parameters that will be stored in the
    ...
    if opt.train_from:
        ...
        # when we loaded an existing models. This should be at least the case
...
class Optimizer(object):
    ...
    def step(self):
        """Update the models parameters based on current gradients.
