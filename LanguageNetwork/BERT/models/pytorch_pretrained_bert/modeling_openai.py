...
def load_tf_weights_in_openai_gpt(model, openai_checkpoint_folder_path):
    """ Load tf pre-trained weights in a pytorch models (from NumPy arrays here)
    """
    import re
    import numpy as np
...
class OpenAIGPTPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, OpenAIGPTConfig):
            raise ValueError(
                f"Parameter config in `{self.__class__.__name__}(config)` should be an instance of class `OpenAIGPTConfig`. To create a models from a pretrained models use `models = {self.__class__.__name__}.from_pretrained(PRETRAINED_MODEL_NAME)`"
            )
        self.config = config
    ...
    def from_pretrained(
        ...
        """
        Instantiate a OpenAIGPTPreTrainedModel from a pre-trained models file or a pytorch state dict.
        ...
        Params:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a pretrained models archive containing:
                    . `openai_gpt_config.json` a configuration file for the models
        ...
        if missing_keys:
            logger.info(
                "Weights of {} not initialized from pretrained models: {}".format(model.__class__.__name__, missing_keys)
            )
        if unexpected_keys:
            logger.info(
                "Weights from pretrained models not used in {}: {}".format(model.__class__.__name__, unexpected_keys)
            )
        if error_msgs:
            raise RuntimeError(
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT models ("Improving Language Understanding by Generative Pre-Training").
    ...
    Params:
        config: a OpenAIGPTConfig class instance with the configuration to build a new models
    ...
    Outputs:
        `hidden_states`: the encoded-hidden-states at the top of the models
    ...
    models = modeling_openai.OpenAIGPTModel(config)
    hidden_states = models(input_ids)
    ```
    """
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT models with a Language Modeling head ("Improving Language Understanding by Generative Pre-Training").
    ...
    models = modeling_openai.OpenAIGPTLMHeadModel(config)
    lm_logits = models(input_ids)
    ```
    """
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT models with a Language Modeling and a Multiple Choice head ("Improving Language Understanding by Generative Pre-Training").
    ...
    models = modeling_openai.OpenAIGPTLMHeadModel(config)
    lm_logits, multiple_choice_logits = models(input_ids, mc_token_ids)
    ```
    """
    ...
    def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None):
        ...
        return losses if losses else (lm_logits, mc_logits)
