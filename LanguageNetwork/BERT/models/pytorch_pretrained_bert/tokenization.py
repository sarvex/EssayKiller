...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_file, do_lower_case=True, max_len=None, do_basic_tokenize=True,
        ...
        if not os.path.isfile(vocab_file):
            raise ValueError(
                f"Can't find a vocabulary file at path '{vocab_file}'. To load the vocabulary from a Google pretrained models use `tokenizer = BertTokenizer.from_pretrained(PRETRAINED_MODEL_NAME)`"
            )
    ...
    def convert_tokens_to_ids(self, tokens):
        ...
        if len(ids) > self.max_len:
            logger.warning(
                f"Token indices sequence length is longer than the specified maximum  sequence length for this BERT models ({len(ids)} > {self.max_len}). Running this sequence through BERT will result in indexing errors"
            )
        return ids
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"Model name '{pretrained_model_name_or_path}' was not found in models name list ({', '.join(PRETRAINED_VOCAB_ARCHIVE_MAP.keys())}). We assumed '{vocab_file}' was a path or url but couldn't find any file associated to this path or url."
            )
            return None
