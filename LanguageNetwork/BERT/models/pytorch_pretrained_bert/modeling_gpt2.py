...
class GPT2PreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, GPT2Config):
            raise ValueError(
                f"Parameter config in `{self.__class__.__name__}(config)` should be an instance of class `GPT2Config`. To create a models from a pretrained models use `models = {self.__class__.__name__}.from_pretrained(PRETRAINED_MODEL_NAME)`"
            )
        self.config = config
    ...
    def from_pretrained(
        ...
        """
        Instantiate a GPT2PreTrainedModel from a pre-trained models file or a pytorch state dict.
        ...
        Params:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a pretrained models archive containing:
                    . `gpt2_config.json` a configuration file for the models
...
class GPT2Model(GPT2PreTrainedModel):
    """OpenAI GPT-2 models ("Language Models are Unsupervised Multitask Learners").

    Params:
        config: a GPT2Config class instance with the configuration to build a new models
    ...
    Inputs:
        ...
        `past`: an optional list of torch.LongTensor that contains pre-computed hidden-states
            ...
            (this is the presents output of the models, cf. below).
    ...
    models = modeling_gpt2.GPT2Model(config)
    hidden_states, presents = models(input_ids)
    ```
    """
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    """OpenAI GPT-2 models with a Language Modeling head ("Language Models are Unsupervised Multitask Learners").
    ...
    models = modeling_gpt2.GPT2LMHeadModel(config)
    lm_logits, presents = models(input_ids)
    ```
    """
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    """OpenAI GPT-2 models with a Language Modeling and a Multiple Choice head ("Language Models are Unsupervised Multitask Learners").
    ...
    models = modeling_gpt2.GPT2LMHeadModel(config)
    lm_logits, multiple_choice_logits, presents = models(input_ids, mc_token_ids)
    ```
    """
    ...
    def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, past=None):
        ...
        return losses if losses else (lm_logits, mc_logits, presents)
