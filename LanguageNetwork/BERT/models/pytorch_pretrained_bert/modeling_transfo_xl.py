...
def build_tf_to_pytorch_map(model, config):
    """ A map of modules from TF to PyTorch.
        This time I use a map to keep the PyTorch models as identical to the original PyTorch models as possible.
    """
    tf_to_pt_map = {}
...
def load_tf_weights_in_transfo_xl(model, config, tf_path):
    ...
    print(f"Weights not copied to PyTorch models: {', '.join(tf_weights.keys())}")
    return model
...
class TransfoXLConfig(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            d_model: Dimensionality of the models's hidden states.
            d_embed: Dimensionality of the embeddings
            d_head: Dimensionality of the models's heads.
...
class TransfoXLPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, TransfoXLConfig):
            raise ValueError(
                f"Parameter config in `{self.__class__.__name__}(config)` should be an instance of class `TransfoXLConfig`. To create a models from a pretrained models use `models = {self.__class__.__name__}.from_pretrained(PRETRAINED_MODEL_NAME)`"
            )
        self.config = config
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        """
        Instantiate a TransfoXLPreTrainedModel from a pre-trained models file or a pytorch state dict.
        ...
        Params:
            pretrained_model_name_or_path: either:
                ...
                - a path or url to a pretrained models archive containing:
                    . `transfo_xl_config.json` a configuration file for the models
...
class TransfoXLModel(TransfoXLPreTrainedModel):
    """Transformer XL models ("Transformer-XL: Attentive Language Models Beyond a Fixed-Length Context").
    ...
    Params:
        config: a TransfoXLConfig class instance with the configuration to build a new models
    ...
    Outputs:
        ...
        `last_hidden_state`: the encoded-hidden-states at the top of the models
    ...
    models = TransfoXLModel(config)
    last_hidden_state, new_mems = models(input_ids)

    # Another time on input_ids_next using the memory:
    last_hidden_state, new_mems = models(input_ids_next, new_mems)
    ```
    """
    def __init__(self, config):
        ...
        if self.attn_type == 0 or self.attn_type != 1 and self.attn_type == 2: # default attention
        ...
        if self.attn_type == 0: # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                if mems_i is not None and mlen > 0:
                    ...
                    if cur_size < mlen:
                        cur_emb_pad = cur_emb[:1].expand(mlen-cur_size, -1, -1)
...
class TransfoXLLMHeadModel(TransfoXLPreTrainedModel):
    ...
    This models add an (adaptive) softmax head on top of the TransfoXLModel
    ...
    # Another time on input_ids_next using the memory:
    last_hidden_state, new_mems = models(input_ids_next, mems=new_mems)
    ```
    """
    def __init__(self, config):
